...
EMULATOR_LOG = ROOT / "tools" / "custom_tcp_requests.jsonl"
...
def wait_for_payload(expected_payload: str, seen_count: int, timeout: float = 8.0) -> None:
    ...
    while time.time() < deadline:
        ...
        for entry in entries[seen_count:]:
            if entry.get("payload") == expected_payload:
                return
        time.sleep(0.25)
    raise RuntimeError(f"TCP payload {expected_payload!r} did not reach emulator")
...
def main() -> None:
    ...
    try:
        ...
        if not rooms:
            _, room = request("POST", "/rooms", token=token, payload={"name": "Custom TCP Test Room", "zone": "Tests"})
            room_id = int(room["id"])
        else:
            room_id = int(rooms[0]["id"])

        external_id = f"custom-tcp-emulator-{int(time.time())}"
        payload = {
            "name": "Custom TCP Emulator Test",
            "roomId": room_id,
            "isOn": False,
            "type": "relay",
            "provider": "custom_tcp",
        ...
        wait_for_payload("POWER ON\n", before)

        before = len(read_emulator_entries())
        request("PUT", f"/devices/{device_id}/toggle", token=token)
        wait_for_payload("POWER OFF\n", before)
