...
EMULATOR_LOG = ROOT / "tools" / "zigbee2mqtt_broker_requests.jsonl"
    ...
    while time.time() < deadline:
        ...
        for entry in entries[seen_count:]:
            if entry.get("kind") == "publish" and entry.get("topic") == topic and entry.get("payload") == expected_payload:
                return
        time.sleep(0.25)
    raise RuntimeError(f"Zigbee2MQTT publish topic={topic!r} payload={expected_payload!r} did not reach emulator")
...
def main() -> None:
    ...
    try:
        ...
        if not rooms:
            _, room = request("POST", "/rooms", token=token, payload={"name": "Zigbee2MQTT Test Room", "zone": "Tests"})
            room_id = int(room["id"])
        else:
            room_id = int(rooms[0]["id"])

        base_topic = "zigbee2mqtt/calhouse_light"
        command_topic = f"{base_topic}/set"
        external_id = f"zigbee2mqtt-emulator-{int(time.time())}"
        payload = {
            "name": "Zigbee2MQTT Emulator Test",
            "roomId": room_id,
            "isOn": False,
            "type": "light",
        ...
        wait_for_publish(command_topic, '{"state":"ON"}', before)

        before = len(read_emulator_entries())
        request("PUT", f"/devices/{device_id}/toggle", token=token)
        wait_for_publish(command_topic, '{"state":"OFF"}', before)
