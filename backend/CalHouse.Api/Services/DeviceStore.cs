...
public class DeviceStore
{
    ...
    public Device AddDevice(string name, string? roomName, int? roomId, bool isOn, string? type, string? provider, Dictionary<string, string>? connection)
    {
        ...
        lock (_sync)
        {
            ...
            command.CommandText = @"
INSERT INTO Devices (Name, RoomId, IsOn, Type, Provider, ConnectionJson, CreatedAt, UpdatedAt)
VALUES (@name, @roomId, @isOn, @type, @provider, @connectionJson, @createdAt, @updatedAt);
SELECT last_insert_rowid();";
            ...
            LogEvent(db, transaction, "info", "api", "DEVICE_CREATED", $"Создано устройство «{cleanName}»", deviceId: insertedId, roomId: resolvedRoomId);
            transaction.Commit();
    ...
    public Device UpdateDevice(int id, string? name, string? roomName, int? roomId, bool? isOn, string? type, string? provider, Dictionary<string, string>? connection)
    {
        lock (_sync)
        {
            using var db = OpenConnection();
...
WHERE Id = @id;";
            ...
            LogEvent(db, transaction, "info", "api", "DEVICE_UPDATED", $"Обновлено устройство «{finalName}»", deviceId: id, roomId: finalRoomId);
            transaction.Commit();
...
ORDER BY d.Name;";
        ...
        while (reader.Read())
        {
            devices.Add(new Device
            {
                ...
                Connection = DeserializeConnection(reader.GetString(7)),
                CreatedAt = ParseUtc(reader.GetString(8)),
                UpdatedAt = ParseUtc(reader.GetString(9)),
            });
        }
